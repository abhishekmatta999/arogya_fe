...
export class LoginComponent implements OnInit {
  ...
  onRegister() {
    if (this.form.valid) {
      this._accountService
        .sendOtp({ email: this.form.value?.email })
        .subscribe({
          next: (res: any) => {
            if (res.success) {
              ...
              this._router.navigate([`${ACCOUNT}/${OTP}`], {
                queryParams: {
                  data: window.btoa(
                    JSON.stringify({ ...this.form.value, type: "register" })
                  ),
                },
              });
            }
  ...
  onRegisterClick() {
    this._router.navigateByUrl(`${ACCOUNT}/${SIGNUP}`);
  }
    ...
    this._accountService.googleLogin(body).subscribe({
      next: (resp: any) => {
        if (resp?.success) {
          ...
          this._toaster.toast("Logged in successfully", "success-toast");
          if (!resp?.data?.user.profile_saved)
